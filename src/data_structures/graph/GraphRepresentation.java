 ...
 * - Implemented as a 2D array (matrix) where cell [i][j] holds a non-zero value (often 1 or vertex weight)
 *   if there is an vertex from vertex i to vertex j.<br>
 * - For an undirected graph, the matrix is symmetric about the diagonal.<br>
 * <br>
 * <b>Advantages:</b><br>
 * - Ideal for dense graphs (where number of edges ≈ V²).<br>
 * - Very fast vertex lookup: O(1) time to check if an vertex exists between two vertices.<br>
 ...
 * <br>
 * <b>When to Use:</b><br>
 * - Use Adjacency Matrix when the graph is dense or when frequent vertex existence checks are needed.
 * - Commonly used in algorithms like Floyd–Warshall (All-Pairs Shortest Path).
 * <p>
 *
 * <b>2. Adjacency Set (or Weighted Adjacency Map):</b><br>
 * - Each vertex maps to a set (or map) of its neighbors along with vertex weights.<br>
