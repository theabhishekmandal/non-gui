 ...
 *
 * For example using the Stats example we want to determine if two stats object have the same average,no matter what type of
 * numeric data each object holds. For example if one object contains the double values 1.0,3.0,2.0 and the other object contains
 * the integer values 2,1 and 3 then the average would be same
 *
